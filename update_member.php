<?php include 'admin_data.php'; 
$m_id = $_GET['m_id'];
$member_data = $sweetsobj->display_one_members($m_id);

?>

<!doctype html>
<html lang="en">
  <head>
    <title>Title</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
  </head>
  <body>
  <div class="container">
         <div class="card w-75 mt-5 mb-5 ">
           <div class="card-header text-center">
           Customer information registration
           </div>
           <div class="card-body">
           <form action="admin_data.php" method="post">
           <div class="row">
             <div class="col-6">
           <p class="mt-3">Enter Fullname</p>
           <input type="text" name="m_username" class="form-control">
           </div>
           <div class="col-6">
           <p class="mt-3">Enter Birthday</p>
           <input type="date" name="m_bday" class="form-control">
           </div>
           </div>
           <div class="row">
           <div class="col-12">
           <p class="mt-3">Enter Password</p>
           <input type="text" name="m_password" class="form-control">
           </div>
           </div>
           <div class="row">
             <div class="col-6">
           <p class="mt-3">Enter Email</p>
           <input type="text" name="m_mail" class="form-control">
           </div>
           <div class="col-6">
           <p class="mt-3">Enter Phone Number</p>
           <input type="text" name="m_tel" class="form-control">
           </div>
           </div>

           <div class="row">
           <div class="col-12">
           <p class="mt-3">Enter Address</p>
           <input type="text" name="m_address" class="form-control">
           </div>
           </div>
           <div class="row">
           <div class="col-12 mt-3">
           <button type="submit" name="update_member" class="btn btn-outline-primary btn-rigth form-control">Update Member</button>
           </div>
           </div>
           </form>
           </div>   
         </div>        
      </div>

    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
  </body>
</html>